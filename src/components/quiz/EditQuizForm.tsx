...
export function EditQuizForm({ quiz }: { quiz: Quiz }) {
  ...
  const form = useForm<QuizFormData>({
    ...
    defaultValues: {
      title: quiz.title,
      // Add stable IDs for react-hook-form to manage array fields
      questions: quiz.questions.map(q => ({...q, id: randomId() })),
    },
  });
  ...
  const onSubmit = async (data: QuizFormData) => {
    ...
    try {
      // Remove the client-side 'id' before sending to Firestore
      ...
      const finalQuizData = {
        ...
        createdAt: quiz.createdAt || serverTimestamp(), // Preserve original creation date
        updatedAt: serverTimestamp(),
      };
      
      const quizDocRef = doc(firestore, 'quizzes', quiz.id);
      await setDoc(quizDocRef, finalQuizData, { merge: true });
  ...
  return (
    <div className="space-y-8">
      <Card>
        <CardHeader>
          <CardTitle className="font-headline">Edit Your Quiz</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form
